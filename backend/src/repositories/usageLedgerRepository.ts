  ...
  async create(entry: NewUsageLedgerEntry): Promise<UsageLedgerRow> {
    const result = await this.db.query<UsageLedgerRow>(
      `INSERT INTO usage_ledger (
        inhouse_api_key_id, vendor_id, vendor_account_id, model_id, workload_id,
        primary_tier_id, fallback_tier_id, is_fallback, status, input_tokens,
        output_tokens, total_tokens, latency_ms, error_category, provider_cost,
